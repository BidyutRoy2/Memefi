// ==UserScript==
// @name         MemeFi by @HiddenGem
// @version      2.1
// @match        https://tg-app.memefi.club/*
// @grant        none
// @icon         https://s2.coinmarketcap.com/static/img/coins/64x64/29373.png
// @downloadURL  x.com/0xaiexp
// @updateURL    x.com/0xaiexp
// @homepage     x.com/0xaiexp
// ==/UserScript==
...
console.log(`${logPrefix}Visit https://t.me/hiddengemnews`, styles.starting);
console.log(`${logPrefix}Visit x.com/0xaiexp`, styles.starting);
...
menuTitle.textContent = 'MemeFi Clicker by @HiddenGem';
...
twitterButton.href = 'https://x.com/0xaiexp';
twitterButton.target = '_blank';
twitterButton.className = 'social-button';
twitterButton.innerHTML = '<img src="data:image/png;base64,">Twitter';
socialButtons.appendChild(twitterButton);

const telegramButton = document.createElement('a');
telegramButton.href = 'https://t.me/hiddengemnews';
...
hiddenUIMessage.innerHTML = 'The script continues to work. UI is hidden to reduce computer load (I hope this helps 😂)';
...
function checkAndClickIconButton() {
  ...
  if (spanElement && /^(Claim|دریافت کنید)$/.test(spanElement.textContent)) {
...
function clickButton() {
  ...
  if (button) {
      ...
      messageBox.textContent = '[MemeFiBot] Spin button not found!!!';
...
function checkAndActivateTurbo() {
  ...
  if (openBoosterButton) {
    setTimeout(() => {
      ...
      console.log(`${logPrefix}Boosters open button clicked`, styles.turbo);
    }, 1000);
  } else {
    console.log(`${logPrefix}Boosters open button not found`, styles.error);
    setTimeout(checkAndActivateTurbo, 5000);
    return;
  }
  ...
  setTimeout(() => {
    ...
    if (boosterCountElement) {
      ...
      console.log(`${logPrefix}Boosters remaining: ${boosterCount}`, styles.turbo);
      if (boosterCount > 0) {
        ...
        if (activateButton) {
          setTimeout(() => {
            ...
            console.log(`${logPrefix}Turbo activation button clicked`, styles.turbo);
            setTimeout(() => {
              ...
              if (confirmButton) {
                setTimeout(() => {
                  ...
                  confirmButton.click();
                  console.log(`${logPrefix}Turbo activated`, styles.turbo);
                  setTimeout(() => {
                    findAndClick();
                    checkAndActivateTurbo();
                  }, Math.random() * 1000 + 13000);
                }, 1000);
              } else {
                console.log(`${logPrefix}Turbo confirmation button not found`, styles.error);
              }
            }, 500);
          }, 1000);
        } else {
          console.log(`${logPrefix}Turbo activation button not found`, styles.error);
        }
      } else {
        console.log(`${logPrefix}No boosters available. Disabling Auto Use Turbo`, styles.info);
        GAME_SETTINGS.autoTurbo = false;
        updateSettingsMenu();
        saveSettings();
      }
    } else {
      console.log(`${logPrefix}Failed to find booster count element`, styles.error);
    }
  }, 1500);
}
